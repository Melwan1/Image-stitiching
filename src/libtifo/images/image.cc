...
namespace tifo::image
{
    ...
    {
        if (horizontal_slices <= 0 || horizontal_slices > width_
            || vertical_slices <= 0 || vertical_slices > height_)
        {
            std::ostringstream oss;
            oss << "tifo::image::Image - rectangular_cut - cannot perform cuts "
                   "with slices count ("
        ...
        for (int horizontal_index = 0; horizontal_index < horizontal_slices;
             horizontal_index++)
        {
            for (int vertical_index = 0; vertical_index < vertical_slices;
                 vertical_index++)
            {
                cut_images[vertical_index * horizontal_slices
                           + horizontal_index] = new PPMImage();
                ...
                cropped_image->get_pixels().resize(cropped_image->width_
    ...
    Image* Image::rebuild_from_cuts(const std::vector<Image*>& cuts,
                                    int horizontal_slices, int vertical_slices)
    {
        if (cuts.size()
                != static_cast<unsigned>(horizontal_slices * vertical_slices)
            || horizontal_slices <= 0 || vertical_slices <= 0)
        {
            throw std::runtime_error("tifo::image::Image - rebuild_from_cuts - "
                                     "cuts size is invalid.");
        }
        PPMImage* image = new PPMImage();
        int cut_width = cuts[0]->get_width();
        int cut_height =
            cuts[0]->get_height(); // suppose every cut has the same size (could
             ...
             horizontal_index++)
        {
                 ...
                 vertical_index++)
            {
                ...
                for (int dx = 0; dx < cut_width; dx++)
                {
                    ...
                    {
                        const std::vector<float>& input_pixel =
                            cuts[vertical_index * horizontal_slices
                                 + horizontal_index]
                                ->get_pixels()[dy * cut_width + dx];
                        std::vector<float>& output_pixel =
                            image
                                ->get_pixels()[(y_min + dy) * image->get_width()
                                               + (x_min + dx)];
                        output_pixel.resize(3);
                        for (int color_index = 0; color_index < 3;
                             color_index++)
