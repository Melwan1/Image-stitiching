...
namespace tifo::image {
    ...
    void PPMImage::read(const fs::path& src_path) {
        ...
        if (!supported_file_formats.contains(file_format)) {
            throw std::runtime_error("tifo::image::PPMImage - the file format " + file_format + " of the source file: " + src_path.string() + " is not supported.");
        }
        ifs >> width_;
        ifs >> height_;
    ...
    void PPMImage::write(const fs::path& dst_path) const {
        if (dst_path.extension() != ".ppm") {
            throw std::runtime_error("tifo::image::PPMImage - the destination file: " + dst_path.string() + " does not have a .ppm extension.");
        }
        std::ofstream ofs(dst_path);
        if (!ofs.is_open()) {
            throw std::runtime_error("tifo::image::PPMImage - the destination file: " + dst_path.string() + " could not be opened.");
        }
        ofs << "P6\n" << width_ << " " << height_ << "\n" << 255 << "\n";
